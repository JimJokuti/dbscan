...
if ( "$order" eq "i" ) {
    ...
    $orderclause = "ORDER BY lc.last_check, i.ipaddrint, i.portnum";
    $orderhr     = "Sorted by last check date, IP, and port number";
}
else {
    $orderclause = "ORDER BY i.ipaddrint, i.portnum";
    $orderhr     = "Sorted by IP and port number";
...
$query =
"SELECT id,subnet, description FROM sc_subnet WHERE addrclass='B' ORDER BY subnetint;";
$sth = $dbh->prepare($query);
$sth->execute();
$sth->bind_columns( undef, \$id, \$subnettoscan, \$subnetdesc );
...
<td><b>SN Desc</b></td><td><b><a title='Click to sort' href=index.pl?$snparams&o=l>Last check</a></b></td><td><b><a title='Click to sort' href=index.pl?$snparams&o=m>Modified</a></b></td></tr>\n";
        ...
        $query2 =
"SELECT i.id,i.ipaddr, i.hostname, i.portnum, lc.last_check,i.modified, s.subnet, s.description, v.val AS mysql_version,v2.val as srhostname, v3.val as verscom,i.notes,i.all_schemas
           FROM sc_instance i LEFT JOIN sc_subnet s ON s.subnet = SUBSTRING_INDEX(i.ipaddr, '.', 3)
           LEFT JOIN sc_variable v ON v.iid=i.id  AND v.keyname = 'version_num' left join sc_variable v2 on v2.iid=i.id AND v2.keyname='hostname' left join sc_variable v3 on v3.iid=i.id and v3.keyname='version_comment'   LEFT JOIN sc_lastcheck lc 
               ON lc.iid = i.id 
               WHERE s.addrclass = 'C'  "
          . $thisclause
        ...
        $sth2->bind_columns(
            undef,        \$id,\$ipaddr, \$hostname,    \$portnum, \$lastcheck,
            \$modified,   \$subnet, \$description, \$mysqlversion,
            \$srhostname, \$verscom,\$notes,\$allschemas
        );
        ...
        $allschemas=substr $allschemas, 0, 25 ;
        chomp($allschemas);
        if (length($allschemas) >= 25) {
          $allschemas.="...";
          }

            $out .= "<tr bgcolor=$bgcolor><td><a class=srv href=serverdetails.pl?s=$id title='Click for server details'>$ipaddr:$portnum</a></td><td><a class=srv href=serverdetails.pl?s=$id title='Click for server details'>$displayhostname</a></td></td><td>$mysqlversion $verscom</td><td>$subnet</td><td>$allschemas</td><td>$description</td><td>$lastcheck</td><td>$modified</td></tr>\n";

        }

        $out .=
"<tr bgcolor=white><td colspan=8>$divider Total hosts known: $hcounter</td></tr>";
...
#            $subnetnumdis = $subnetnum . ".0";
        ...
        #print $counter;
        $out .= "$subnetdesc,($subnetnumdis/24)\n";
        $out .=
...
$whereclause="  
 ...
 ";
    ...
    $query =
"SELECT s.id,s.subnet, s.description,i.id,i.ipaddr,i.hostname,i.portnum,v.val as mysqlversion,v2.val as srhostname,v3.val as verscom,lc.last_check,i.modified,i.notes,i.all_schemas FROM sc_subnet s left join sc_instance i on i.subnetint=s.subnetint 
left join sc_variable v on v.iid=i.id and v.keyname='version' left join sc_variable v2 on v2.iid=i.id and v2.keyname='hostname' left join sc_variable v3 on v3.iid=i.id and v3.keyname='version_comment'   LEFT JOIN sc_lastcheck lc 
    ON lc.iid = i.id 
    WHERE 1=1 $whereclause $orderclause";

#    print "$query";
    
$out.=    "<tr bgcolor=silver><td><b><a href=index.pl?str=$string&o=i title='Click to sort'>IP Address</a></b></td><td><b><a href=index.pl?str=$string&o=h title='Click to sort'>DNS hostname (Instance reported hostname)</a></b></td></td><td><b><a title='Click to sort' href=index.pl?str=$string&o=v>MySQL version</a></b></td><td><b><a title='Click to sort' href=index.pl?str=$string&o=s>Subnet</a></b></td>
<td><b>Schemas</b></td>
<td><b>SN Desc</b></td><td><b><a title='Click to sort' href=index.pl?str=$string&o=l>Last check</a></b></td><td><b><a title='Click to sort' href=index.pl?str=$string&o=m>Modified</a></b></td></tr>\n";

    
    $sth2 = $dbh->prepare($query);
    $sth2->execute() or die($DBI::errstr);
    $sth2->bind_columns( undef, \$subnetid, \$subnetnum, \$subnetdesc,\$id,\$ipaddr, \$hostname,    \$portnum,\$mysqlversion,
               \$srhostname, \$verscom ,\$lastcheck,\$modified,\$notes,\$allschemas);
    ...
    $counter = 0;
        ...
        if (length($allschemas) >= 25) {
          $allschemas.="...";
            ...
            $out .= "<tr bgcolor=$bgcolor><td><a class=srv href=serverdetails.pl?s=$id title='Click for server details'>$ipaddr:$portnum</a></td><td><a class=srv href=serverdetails.pl?s=$id title='Click for server details'>$displayhostname</a></td></td><td>$mysqlversion $verscom</td><td>$subnetnum</td><td>$allschemas</td><td>$subnetdesc</td><td>$lastcheck</td><td>$modified</td></tr>\n";
