...
print $divider;
print
"<a href=\"mailto:rbyrd\@riotgames.com?subject=MySQL Audit suggestion\">Email Richard a suggestion for improvement</a>";
...
"<tr bgcolor=silver><td><b><a href=index.pl?$snparams&o=i title='Click to sort'>IP Address</a></b></td><td><b><a href=index.pl?$snparams&o=h title='Click to sort'>DNS hostname (Instance reported hostname)</a></b></td></td><td><b><a title='Click to sort' href=index.pl?$snparams&o=v>MySQL version</a></b></td><td><b><a title='Click to sort' href=index.pl?$snparams&o=s>Subnet</a></b></td><td><b>SN Desc</b></td><td><b><a title='Click to sort' href=index.pl?$snparams&o=m>Modified</a></b></td></tr>\n";
        ...
        $query2 =
"SELECT i.id,i.ipaddr, i.hostname, i.portnum, i.modified, s.subnet, s.description, v.val AS mysql_version,v2.val as srhostname, v3.val as verscom
           FROM sc_instance i LEFT JOIN sc_subnet s ON s.subnet = SUBSTRING_INDEX(i.ipaddr, '.', 3)
           LEFT JOIN sc_variable v ON v.iid=i.id left join sc_variable v2 on v2.iid=i.id left join sc_variable v3 on v3.iid=i.id WHERE s.addrclass = 'C' AND v.keyname = 'version_num' AND v2.keyname='hostname' and v3.keyname='version_comment' "
          . $thisclause
...
$whereclause=" AND concat(s.subnet,s.description,i.ipaddr,i.hostname,i.portnum,v.val,v2.val,v3.val) LIKE '%$string%' ";

    $query =
"SELECT s.id,s.subnet, s.description,i.id,i.ipaddr,i.hostname,i.portnum,v.val as mysqlversion,v2.val as srhostname,v3.val as verscom,i.modified FROM sc_subnet s left join sc_instance i on i.subnetint=s.subnetint
