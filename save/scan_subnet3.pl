...
#        }
        ...
        if ( "$crap2" =~ "$thisip" ) {
            ...
            $query =
"INSERT INTO sc_instance (ipaddr,ipaddrint,hostname,portnum) VALUES ('$thisip',INET_ATON(ipaddr),'$thishost','$thisport') ON DUPLICATE KEY UPDATE hostname='$thishost',portnum='$thisport';";
...
$query =
"SELECT  id,hostname,portnum FROM sc_instance where ipaddr like '$thissn' and checkflag='Y' ORDER BY hostname";
print $query;
...
#print "here";
    ...
    if ( !$dbhr ) {
        ...
        if ( $key eq "version" ) {
          ...
          $queryii="INSERT INTO sc_variable (iid,keyname,val) VALUES ($bid,'version_num','$thisvers') ON DUPLICATE KEY UPDATE val='$thisvers';";
          $sth = $dbh->prepare($queryii);
          $sth->execute(); 
          $queryii="INSERT INTO sc_variable (iid,keyname,val) VALUES ($bid,'version_type','$thistype') ON DUPLICATE KEY UPDATE val='$thistype';";
          $sth = $dbh->prepare($queryii);
          $sth->execute(); 
          ...
          $queryii="INSERT INTO sc_variable (iid,keyname,val) VALUES ($bid,'version_int',INET_ATON('$thisversint')) ON DUPLICATE KEY UPDATE val=INET_ATON('$thisversint');";
          $sth = $dbh->prepare($queryii);
          $sth->execute(); 
...
# do later        
#        $thisverscalc=$thisvers;
#        $thisverscalc=~s/[a-z]//g;

        chop($queryi);
        $queryi.=" ON DUPLICATE KEY UPDATE val=VALUES(val);";

#        $queryi =
#"UPDATE sc_instance SET mysql_version_num='$thisvers',mysql_version_type='$thistype',mysql_version_int=INET_ATON('$thisverscalc'),mysql_version_comment='$thisverscom',mysql_version_arch='$thisverscm',mysql_version_os='$thisversco' where id=$bid;";

#        print "\n$queryi\n";
        $sth = $dbh->prepare($queryi);
        $sth->execute();
        
        # now update the nummeric version
        
 #       ($thisvers,$thistype)=split(/\-/,$val);
        
#        $queryi="INSERT INTO sc_variable (iid,keyname,val) VALUES ($bid,version_num,SUBSTRING_INDEX(val,'-',1) FROM sc_variable WHERE keyname='version';
