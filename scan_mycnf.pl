...
# now the OS
    ...
    $oscnt=0;
#    $cmd = "bash ./grabos.sh " . $hip;
    system($cmd);
    open OS,"version.data";
    while (<OS>) {
      $oscnt++;
      if ($oscnt==1) { $osname=$_;}
      if ($oscnt==2) { $osvers=$_;}
    ...
    print $osname;
    if ("$osname" !~ /denied/ ) {
      $queryo="INSERT INTO sc_osvers (iid,osname,osvers) VALUES ($hid,'$osname','$osvers') ON DUPLICATE KEY UPDATE osname='$osname',osvers='$osvers';";
        $sth3 = $dbh->prepare($queryo);
        $sth3->execute();
    }
