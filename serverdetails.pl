...
<form action=serverdetails.pl><font size=-1>Choose another instance or diff two servers' config: $dd1&nbsp;$dd2 <input type=submit value=Go></form>
</td>";

if ($FORM{'mcd'} eq "" ) {
$header2 .=
"<td valign=bottom>
<form action=serverdetails.pl><font size=-1><input name=s value=$FORM{'s'} type=hidden><input name=mcd value=1 type=hidden><input type=submit value='Diff current vs my.cnf for this instance'></form>
</td>";
}
...
sub gen_report() {
    ...
    $query =
"SELECT i.id,i.ipaddr,i.hostname,i.portnum,i.subnet,i.notes,i.modified,s.description,i.all_schemas FROM sc_instance i LEFT JOIN sc_subnet s ON s.subnetint=i.subnetint $serverclause";
...
"<table class=var-table><tr bgcolor=silver><td><b>Variable</b></td><td class=var><b>Value</b></td></tr>\n"
        ...
        $out .=
         "<tr><td colspan=4 bgcolor=#F2F2F2 align=center><b><font color=darkblue>Schemas in this instance: &nbsp;&nbsp;$allschemas</font></b></td></tr>\n";
...
"<table class=var-table align=center><tr bgcolor=silver><td><b>Variable</b></td><td><b>$hostname:$portnum<br>($srhostname/$ipaddr)</b></td><td><b>$hostname2:$portnum2<br>($srhostname2/$ipaddr2)</b></td></tr>\n"
    ...
    $out .=
"<tr><td colspan=4><b>$numrows differences between these instances</b></td></tr>\n";
    $out .=
...
FROM
  sc_variable v1 
  LEFT JOIN sc_mycnf v2 
    ON v1.keyname = v2.keyname 
    AND v1.val != v2.val 
WHERE v1.iid = $sid 
  AND v2.iid = v1.iid 
  AND SUBSTR(v2.val, LENGTH(v2.val), 1) != 'K' 
  AND SUBSTR(v2.val, LENGTH(v2.val), 1) != 'M' 
  AND SUBSTR(v2.val, LENGTH(v2.val), 1) != 'G')
UNION
(SELECT 
  v1.keyname kn,
  v1.val,
...
(SELECT 
  ...
  v1.val,
  v2.keyname,
  v2.val * 1024 * 1024 * 1024 
FROM
  sc_variable v1 
  LEFT JOIN sc_mycnf v2 
    ON v1.keyname = v2.keyname 
    AND v1.val != v2.val * 1024 * 1024 * 1024 
WHERE v1.iid = $sid 
  AND v2.iid = v1.iid 
  AND SUBSTR(v2.val, LENGTH(v2.val), 1) = 'G') 
ORDER BY kn;";
...
"<tr><td bgcolor=$bgcolor>$keyname1</td><td bgcolor=$bgcolor class=var>$val1</td><td bgcolor=$bgcolor class=var>$val2</td></tr>\n";
    ...
    $diffout =
"<table class=var-table align=center><tr bgcolor=silver><td><b>Variable</b></td><td><b>$hostname:$portnum<br>($srhostname/$ipaddr)<br><font color=purple>RUNNING CONFIGURATION</b></td><td><b>$hostname:$portnum<br>($srhostname/$ipaddr)<br><font color=darkgreen>my.cnf settings</b></td></tr>\n"
      . $diffout
      . "</table>";

    $out .=
"<tr><td colspan=4><b>$numrows differences between the current running configuration of $hostname:$portnum and its configuration file:</b></td></tr>\n";
    $out .=
"<tr><td width=50% valign=top style=\"text-wrap:normal;word-wrap:break-word;\">$diffout</td></tr>";
