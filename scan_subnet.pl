...
$querystart="INSERT INTO sc_scaninfo (type,scanwhat,created) VALUES ('start','$thissubnet',NULL) ;";
$sth = $dbh->prepare($querystart);
$sth->execute();
...
system("/bin/rm $thisfilebase.out $thisfilebase.err");
...
"INSERT INTO sc_variable (iid,keyname,val) VALUES ($bid,'version_int',INET_ATON('$thisversint')) ON DUPLICATE KEY UPDATE val=INET_ATON('$thisversint');";
            ...
            $queryii =
"INSERT INTO sc_lastcheck (iid,last_check) VALUES ($bid,now()) ON DUPLICATE KEY UPDATE last_check=now();";
            $sth = $dbh->prepare($queryii);
            $sth->execute();
...
$queryii="DELETE FROM sc_schema where iid=$bid";
            $sth = $dbh->prepare($queryii);
            $sth->execute();



    $queryr = "SELECT DISTINCT(SCHEMA_NAME) FROM information_schema.SCHEMATA WHERE SCHEMA_NAME NOT IN ('information_schema','mysql','performance_schema','test') ORDER BY SCHEMA_NAME;";
...
$sthr->execute();
$sthr->bind_columns( undef, \$dbname );
while ( $sthr->fetch() ) {
            $queryii =
"INSERT INTO sc_schema (iid,schemaname) VALUES ($bid,'$dbname') ON DUPLICATE KEY UPDATE schemaname='$dbname';";
            $sth = $dbh->prepare($queryii);
            $sth->execute();
...
$querydone3="UPDATE sc_variable SET val = '1' WHERE val = 'ON' ;";
$sth = $dbh->prepare($querydone3);
$sth->execute();

$querydone4="UPDATE sc_variable SET val = '0' WHERE val = 'OFF' ;";
$sth = $dbh->prepare($querydone4);
$sth->execute();

$querydone5="UPDATE sc_instance i SET all_schemas=(SELECT GROUP_CONCAT(schemaname) FROM sc_schema sch WHERE sch.iid=i.id)";
$sth = $dbh->prepare($querydone5);
$sth->execute();

$querydone5="INSERT INTO sc_scaninfo (type,scanwhat,created) VALUES ('end','$thissubnet',NULL) ;";
$sth = $dbh->prepare($querydone5);
$sth->execute();
